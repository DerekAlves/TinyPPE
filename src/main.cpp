...
void convertRGB565toRGB888(uint8_t* rgb565, uint8_t* rgb888, size_t pixelCount) {
...
void loop()
{
    ...
    // Print the prediction results (classification)
        ...
        //ei_printf("SETCOL:\r\n");
        int IMAGE_SIZE = int(EI_CLASSIFIER_INPUT_WIDTH*EI_CLASSIFIER_INPUT_HEIGHT*3);
        float classificationResults[EI_CLASSIFIER_LABEL_COUNT];
        //Serial.print("Image Size:");
        //Serial.println(IMAGE_SIZE);

        //ei_printf("GOTRES:\r\n");

        uint8_t out_buf[EI_CLASSIFIER_INPUT_WIDTH * EI_CLASSIFIER_INPUT_HEIGHT * 3];
        convertRGB565toRGB888(snapshot_buf, out_buf, EI_CLASSIFIER_INPUT_WIDTH * EI_CLASSIFIER_INPUT_HEIGHT);

        // Preencha imageData com os dados da imagem
        //ei_printf("GOTIMG:\r\n");

        // Preencha classificationResults com os resultados da classificação
        for (size_t i = 0; i < EI_CLASSIFIER_LABEL_COUNT; i++) {
            classificationResults[i] = result.classification[i].value;
        }
        //ei_printf("SENDDATA:\r\n");
        Serial.write(out_buf, IMAGE_SIZE);
        Serial.write((uint8_t *)classificationResults, (EI_CLASSIFIER_LABEL_COUNT * sizeof(float)));
